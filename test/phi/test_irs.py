...
import pandas as pd
import numpy as np
from scripts.utils.utils.utils import IRS, ZeroBond, FinanceUtils
...
def test_irs(data, simple_params):
    ...
    # Real values
    v_real = IRS.IRS_test(xn=xt, t=dt, Ti=T, Tm=Tm, ct=ct)
    ...
    with tf.GradientTape() as tape:
        y = IRS.IRS_test_normalized(xn=xt, t=dt, Ti=T, Tm=Tm, ct=ct)
    grad_df = tape.gradient(y, {"xn": xt})
    grads = grad_df["xn"]
    # Custom grads
    h = 1e-14
    x_p_h = IRS.IRS_test_normalized(xn=xt + h, t=dt, Ti=T, Tm=Tm, ct=ct)
    x_m_h = IRS.IRS_test_normalized(xn=xt - h, t=dt, Ti=T, Tm=Tm, ct=ct)
    ...
    for i in range(1, nsteps):
        ...
        v_custom[:, i] = (
            v_custom[:, i - 1] + grad_custom_reshaped[:, i - 1] * delta_x_reshaped[:, i]
        )
