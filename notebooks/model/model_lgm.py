...
# Utils
from utils.utils.utils import FinanceUtils, MathUtils
...
class LGM_model(tf.keras.Model):
    ...
    def custom_train_step(self, X, y=None, batch=0, epoch=0, start_time=None):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.loss_lgm(
                x=X, v=v, predictions=predictions, N_steps=self.N, verbose=self._verbose
            )
    ...
    def get_losses_internal(self):
        ...
        """
        return {
            "strike_loss": float(self._loss_tracker_t1.result()),
    ...
    def _get_dv_dx(self, X):
        ...
        self._grads = MathUtils.custom_diagonal_derivative(X, self._custom_model)[
            :, :, 0
        ]
        self._grads = tf.reshape(X, (x_dim, y_dim))
        # Verbose to output
        if self._verbose:
    ...
    def loss_lgm(
        ...
        strike_loss = tf.math.squared_difference(prediction[:, -1], function_at_strike)
        # Autodiff f
        xn = tf.Variable(x[:, -1, 0], name="xn", trainable=True)
        tn = tf.Variable(
            np.float64(self._deltaT * len_path), name="tn", trainable=False
        )
        ...
        derivative_loss = tf.math.squared_difference(
            self._get_dv_dxi(n_idx - 1), df_dxn
        )
        # Epoch error per step
        error_per_step = (
            tf.reduce_sum(
                tf.math.squared_difference(v[:, 1:], predictions[:, 1:]), axis=1
            )
            / N_steps
        )
    ...
    def predict(self, X):
        ...
        for i in range(0, self.N - 1):
            second_term = tf.math.multiply(
                tf.reshape(grads[:, i], (samples, 1)),
                tf.reshape(tf.math.subtract(X[:, i + 1, 0], X[:, i, 0]), (samples, 1)),
            )
            v[:, i + 1, :] = tf.math.add(predictions[:, i, :], second_term)
