...
class FinanceUtils:
    ...
    def C(t, sigma_value=0.0075):
        ...
        return integrate.quad(
            lambda x: FinanceUtils.sigma(t=x, sigma_0=sigma_value) ** 2, 0, t
        )[0]
...
class ZeroBond:
    ...
    def Z_tensor(xt, t, T, ct=None):
        ...
        return (
            ZeroBond.D_tensor(T)
            * tf.math.exp(
                -0.5 * ZeroBond.H_tensor(T) ** 2 * ct - ZeroBond.H_tensor(T) * xt
            )
            * ZeroBond.N_tensor(t, xt, ct)
        )
...
class IRS:
    ...
    def IRS_normalized(xn, T, TN, ct, period=6, K=0.03):
        ...
        variable = 1 - ZeroBond.Z_tensor(xn, T, TN, ct)
        fixed = tf.zeros(tf.shape(xn), dtype=tf.float64)
...
class Swaption:
    ...
    def par_swap(
        ...
        fixed = Swaption.anuality_swap(xn, Ti, Tm, ct, period)
        return (pi - pm) / (fixed)

    @staticmethod
    def positive_part_parswap(xn, Ti, Tm, ct, period=6, K=0.03):
        par_swap = Swaption.par_swap(xn, Ti, Tm, ct, period)
        return np.maximum(0, par_swap - K)

    @staticmethod
    def density_normal(xT, xn, ct, cT):
        mu = xn
        std = np.sqrt(cT - ct)
    ...
    def Swaption_test(
        ...
        def integra_swap(xT, xnj, ct, cT):
            ...
            density_normal = Swaption.density_normal(xT, xnj, ct=ct, cT=cT)
            anuality_term = Swaption.anuality_swap(xT, Ti, Tm, cT, period)
        ...
        for t_unique in ts_unique:
            ct_dict[t_unique] = FinanceUtils.C(t_unique, sigma_value)
        cT = FinanceUtils.C(Ti, sigma_value)
        swaption_results = []
        # TODO: Clean
        if debug == True:
            i = t == 0
            ...
            Ti = np.float64(Ti)
            Tm = np.float64(Tm)
            v_zero = Swaption.Swaption_at_zero(cT, Ti, Tm, period=period, K=K)
    ...
    def Swaption_at_zero(
        ...
        second_term = -ZeroBond.D(TN) * norm.cdf(
            -(y_star + deltaHN * ceta_T) / np.sqrt(ceta_T), 0, 1
        )
        # Third term
        third_term = 0
        for i in range(1, num_times + 1):
            ...
            third_term += ZeroBond.D(Ti) * norm.cdf(
                -(y_star + deltaHi * ceta_T) / np.sqrt(ceta_T), 0, 1
            )
    ...
    def Swaption(xn, T, TN, ct, period=6, K=0.03):
        ...
        tensor_irs = tf.reshape((variable + fixed), (variable.shape[0], 1))
        zero_mask = tf.zeros((tensor_irs.shape[0], 1), dtype=tensor_irs.dtype)
        tensor_irs = tf.concat([tensor_irs, zero_mask], axis=1)
