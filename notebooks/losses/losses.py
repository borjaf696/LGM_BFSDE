...
class Losses:
    ...
    def loss_lgm(
        ...
        strike_loss = tf.reshape(
            tf.math.abs(predictions[:, -1] - phi(xn_tensor, tn, T, ct)), (batch_size, 1)
        )
        # Repeat the tensor to adapt dimensions
        strike_loss = tf.tile(strike_loss, tile_multiples)
        ...
        derivative_loss = tf.reshape(
            tf.math.squared_difference(derivatives, df_dxn), (batch_size, 1)
        )
        # Repeat the tensor to adapt dimensions
        derivative_loss = tf.tile(derivative_loss, [1, N_steps])
        derivative_loss_reshaped = tf.reshape(derivative_loss, [-1])
        # Epoch error per step
        error_per_step = (
            tf.cumsum(tf.math.squared_difference(v, predictions), axis=1) / N_steps
        )
        # Flatten the cumsum
        ...
        loss_per_sample = tf.math.add(
            error_per_step, tf.math.add(strike_loss_reshaped, derivative_loss_reshaped)
        )

        # Apply mask to only change given the last step
        return tf.math.multiply(loss_per_sample, mask_loss), losses_trackers
